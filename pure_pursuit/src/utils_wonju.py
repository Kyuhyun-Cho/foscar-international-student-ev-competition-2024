...
class purePursuit: ## purePursuit 알고리즘 적용 ##
    ...
    def checkDynamicObstacle(self, clock_wise, min_distance):
        ...
        return is_dynamic_obstacle, False, distance_threshold, 0.0
    
    def checkDynamicObstacleInScurve(self, min_distance):
        is_dynamic_obstacle = False
        is_slowdown_obstacle = False

        # final_control_mode = 'DEFAULT'
        distance_threshold_big = 6.0 # 4.5
        distance_threshold_small = 2.5

        if min_distance <= distance_threshold_small:
            is_dynamic_obstacle = True
            is_slowdown_obstacle = False
            # final_control_mode = 'STOP'
        elif min_distance <= distance_threshold_big:
            # final_control_mode = 'SLOW'
            is_dynamic_obstacle = False
            is_slowdown_obstacle = True
        
        return is_dynamic_obstacle, is_slowdown_obstacle, distance_threshold_small, distance_threshold_big
...
def rotateLiDAR2GPS(fusion_result, vehicle_status) :
    ...
    for bbox in fusion_result :
        ...
        if (115.42681589194162 <= new_x <= 116.42681589194162) and (150.69836558116663 <= new_y <= 151.69836558116663):
            continue
