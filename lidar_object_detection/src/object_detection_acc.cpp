...
void cfgCallback(lidar_object_detection::objectDetectorAccConfig &config_acc, int32_t level) {
    xMinROI = config_acc.xMinROI;
    xMaxROI = config_acc.xMaxROI;
    yMinROI = config_acc.yMinROI;
    yMaxROI = config_acc.yMaxROI;
    zMinROI = config_acc.zMinROI;
    zMaxROI = config_acc.zMaxROI;

    minPoints = config_acc.minPoints;
    epsilon = config_acc.epsilon;
    minClusterSize = config_acc.minClusterSize;
    maxClusterSize = config_acc.maxClusterSize;

    xMinBoundingBox = config_acc.xMinBoundingBox;
    xMaxBoundingBox = config_acc.xMaxBoundingBox;
    yMinBoundingBox = config_acc.yMinBoundingBox;
    yMaxBoundingBox = config_acc.yMaxBoundingBox;
    zMinBoundingBox = config_acc.zMinBoundingBox;
    zMaxBoundingBox = config_acc.zMaxBoundingBox;

    leafSize  = config_acc.leafSize;

    maxIterations = config_acc.maxIterations;
    distanceThreshold = config_acc.distanceThreshold;
}
...
int main (int argc, char** argv) {
    // Initialize ROS
    ros::init (argc, argv, "lidar_object_detection_acc");
    ros::NodeHandle nh;

    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorAccConfig> server;
    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorAccConfig>::CallbackType f;
    ...
    pubROI = nh.advertise<sensor_msgs::PointCloud2> ("roi_raw_acc", 1);
    pubCluster = nh.advertise<sensor_msgs::PointCloud2>("cluster_acc", 1);
    pubObjectInfo = nh.advertise<lidar_object_detection::ObjectInfo>("object_info_acc", 1);
    pubObjectShapeArray = nh.advertise<visualization_msgs::MarkerArray>("bounding_box_acc", 1);
    pubSegmentPlaneInfo = nh.advertise<sensor_msgs::PointCloud2> ("segment_plane_acc", 1);
