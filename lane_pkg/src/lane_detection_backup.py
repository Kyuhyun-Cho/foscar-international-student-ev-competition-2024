...
class  LaneDetection:
    def __init__(self):
        ...
        self.stopline_flag_pub = rospy.Publisher('/stopline_flg', Bool, queue_size=1)
        ...
        self.up_hist_end_line = 500
        self.down_hist_start_line = 500
        ...
        while not rospy.is_shutdown():
            ...
            if len(self.img)!= 0:
                ...
                stopline_indices = np.where(down_hist > 500)[0] + self.down_hist_start_line
                ...
                if self.current_waypoint >= 2250:
                    ...
                    if self.current_lane == 3:
                        ...
                        if self.avoid_status == 'avoiding':
                            ...
                            if self.heading >= self.gt_heading - 15:
                                self.avoid_x_location += 1
                                self.center_index = self.avoid_x_location
                                angle = self.center_index - 640

                            elif self.heading < self.gt_heading - 15:
                                self.avoid_x_location  = 640
                                self.avoid_status= 'returning'

                        elif self.avoid_status == 'returning':
                            if self.heading < self.gt_heading-6:
                                self.avoid_x_location -= 0.5
                                self.center_index = self.avoid_x_location
                                angle = self.center_index - 640

                            elif self.heading >= self.gt_heading-6:
                                self.current_lane += 1
                                self.avoid_status = 'lanekeeping'
